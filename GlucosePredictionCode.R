...
columnCleaning <- function(OriginalData){
  ...
  number.column <- ncol(dataset)
  number.row <- nrow(dataset)
  name <- as.vector(colnames(dataset))
  for(i in 1:number.column){
    na <- sum(is.na(dataset[, i]))
    percetage.missing <- na / number.row
    if(percetage.missing > 0.5){ #评判值
      dl.variable <- as.character(name[i])
...
RandomforestModel <- function(dataset){
  ...
  set.seed(1)
  train.set <- dataset[sample(1:nrow(dataset), 
                       nrow(dataset)%/%3*2, 
                       replace = FALSE), ]
  test.set <- setdiff(dataset, train.set)
  model <- randomForest(train.set[, 1:(y.index - 1)], 
                        train.set[,y.index], 
                        replace=T, 
                        keep.forest = T)
  return(model)
...
ImportanceRank <- function(RFModel){
  ...
  index.tmp <- order(feature.importance$weights, 
                     decreasing = T)
...
GenerateBestData <-function(data, feautre.importance){
  ...
  repeat{
    ...
    set.seed(1)
    train.set <- data[sample(1:nrow(data), 
                      nrow(data)%/%3*2, 
                      replace = FALSE), ]
    test.set <- setdiff(data, train.set)
    result.preditc <- predict(rf.model, 
                              test.set[, 1:n])
    truth<-test.set$葡萄糖
